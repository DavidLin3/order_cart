...
from app.models import Shop, Orders, OrderCart
from mongoengine.queryset.visitor import Q
from bson import ObjectId
from app.taobao_query import taobao_query
from app.add_taobao_cart import add_taobao_cart
...
@taobao.route('/tbCart/shopList', methods=['GET', 'POST'])
def get_shop_list():
    '''
    商品库信息组合去重及商品库模糊搜索功能
    '''
    ...
    if query_set:    # 将查询结果去重并保存至shop中
        ...
        for item in only_records:
            ...
            if not max_id:    # 商品库为空，直接保存
                ...
                if not Shop.objects(goods_url=item['goods_url'], goods_sku1=item['goods_sku1'], goods_sku2=item['goods_sku2']):
                    Shop(**item).save()

    # 商品库展示
    if ww_id or store_name or goods_title or sku_id:    # 条件查询（模糊查询）
        page_data = Shop.objects(Q(__raw__={'ww_name':{'$regex':ww_id}})
...
@taobao.route('/tbCart/saveSku', methods=['GET', 'POST'])
@user_scan_req
def save_sku():
    '''
    保存sku至shop商品库
...
@taobao.route('/tbCart/addOrderList', methods=["GET", "POST"])
@user_scan_req
def add_order_list():
    '''
    批量或逐个加入采购单
    ...
    # 查询参数
    ids_json = request.args.get('ids') or '{"ids": ["5b7a6a093f3e1cceb46c84da", "5b7a6a023f3e1cceb46c84c1", "5b7a6c943f3e1cceb46c9f0e"]}'
    ...
    if len(OrderCart.objects) < 50:
        for id in ids:
            if not OrderCart.objects(_id=ObjectId(id)):    # 排除重复加入的订单
                order_dict = Shop.objects(_id=ObjectId(id)).first().to_mongo().to_dict()
...
@taobao.route('/tbCart/orderList', methods=["GET", "POST"])
def get_order_list():
    '''
    采购单列表页展示
    '''
    ...
    page_data = OrderCart.objects.paginate(page=page_num, per_page=page_size)
    list_all_query = []
    for item in page_data.items:
        new_each_result = OrderedDict()
        for k, v in MAPPING.items():
...
@taobao.route('/tbCart/searchView', methods=["GET", "POST"])
@user_scan_req
def search_view():
    '''
    采购单关键词联想搜索
    ...
    shop_list = []
    if goods_title:
        queryset = Shop.objects(__raw__={'goods_title':{'$regex':goods_title}})    # 查询商品名称
        for ob in queryset:
            ...
            if len(shop_list) == 10:    # 取前10条记录
                break
    if sku_id:
        queryset = Shop.objects(__raw__={'sku_id': {'$regex': sku_id}})    # 查询商家sku
        for ob in queryset :
            ob_dict = ob.to_mongo().to_dict()
            shop_dict = {'id': str(ob_dict['_id']), 'goodsTitle': ob_dict['goods_title'], 'skuId': ob_dict['sku_id']}
            shop_list.append(shop_dict)
...
@taobao.route('/tbCart/addCart', methods=['GET', 'POST'])
@user_scan_req
def add_cart():
    '''
    加入淘宝购物车
    '''
    cart_parm_json = request.args.get('cartParm') or '{"cartParm": [{"id": "5b7a6a093f3e1cceb46c84da", "quantity": "2"}, {"id": "5b7a6a023f3e1cceb46c84c1", "quantity": "5"}]}'   # json形式的请求参数
    cart_parm_dict = json.loads(cart_parm_json)
    cart_list = cart_parm_dict['cartParm']

    for item in cart_list:    # 查询参数并构造字典列表
        ob = OrderCart.objects(_id=ObjectId(item['id'])).first()
    ...
    order_cart_list = add_taobao_cart(cart_list, cookie)    # 加入淘宝购物车，返回购物车商品信息列表
...
@taobao.route('/tbCart/uploadSku', methods=['GET', 'POST'])
def upload_sku():
    '''
    excel导入卖家sku
    '''
...
@taobao.route('/tbCart/uploadQty', methods=['GET', 'POST'])
@user_scan_req
def upload_quantity():
    '''
    excel导入采购数量
    ...
    if request.method == 'POST':
        ...
        for item in upload_data:
            ob = OrderCart.objects(goods_url=item['商品链接'])
            if not ob:
                not_match_sum += 1
            else:
...
@taobao.route('/tbCart/remove', methods=['GET', 'POST'])
def remove_order():
    '''
    移除采购单商品
    '''
    id = request.args.get('id')
    OrderCart.objects(_id=ObjectId(id)).delete()
    return Response(json.dumps({"status": 0, "msg": "请求成功"}), mimetype='application/json')


base_dir = os.path.abspath(os.path.dirname(__file__))
@taobao.route('/qrcode', methods=['GET', 'POST'])
def get_QR_url():
    cmd_path = os.path.join(base_dir, '../')
    if sys.platform == 'win32':
        cmd = 'python ' + cmd_path + '/prepare_start.py'
    else:
        cmd = 'python3 ' + cmd_path + '/prepare_start.py'
...
@taobao.route('/isLogin', methods=['GET', 'POST'])
def get_isLogin():
    user_login_info_dict = None
    if 'QR' in session:
        user_login_info_dict = RedisClient().get_login_info_by_img(session['QR'])
...
@taobao.route("/logout/", methods=["GET", "POST"])
def logout():
